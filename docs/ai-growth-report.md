...
聚合报表还会在增长归因之后生成 `cross_metric_diagnoses`。这一确定性层按
业务结果、增长驱动、漏斗瓶颈和矛盾信号组织每个维度的完整经营链路，并
识别流量下降但效率提升、流量增长但支付未同步、同比改善但环比转弱等组合
模式。`priority_score` 综合结果恶化、流量与支付规模、漏斗偏离、同比环比
恶化、矛盾信号和增长机会；AI 只能解释该排序，不能覆盖后端模式或瓶颈。

当报表没有安全总计行时，`cross_metric_summary.scope` 为
`dimension_only`。此时核心结论必须明确从主要维度表现出发，不生成伪造的
整体经营结论。同一重点问题引用的证据也必须绑定同一个业务维度，最终问题
顺序由后端按 `priority_score` 校正。

`growth_explanation` 草稿不包含 `dimension_value` 或 `growth_driver`。服务端
要求归因证据只绑定一个维度，并从对应的 `growth_attribution` 注入这两个
权威字段。模型即使额外返回同名字段也会被忽略，不能覆盖后端结果。若引用
无法唯一绑定维度、跨维度或缺少对应漏斗节点证据，仅丢弃增长来源说明并在
`limitations` 记录降级，不影响其他已经通过校验的报告内容。

## Prompt 设计

System Prompt 将角色、任务、格式与事实约束分开定义，核心规则为：

1. aggregate_metrics 优先使用后端生成的多指标联合诊断；
2. 不重新计算或修改任何指标；
